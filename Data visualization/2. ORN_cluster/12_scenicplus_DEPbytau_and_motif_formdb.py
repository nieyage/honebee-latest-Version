...
adata.write('/md01/nieyg/project/honeybee/honebee-latest-Version/09_GRN/02_SCENIC/04_scenicplus/01_scRNA/adata_subcluster.h5ad', compression='gzip')
...
pickle.dump(bed_paths,
            open(os.path.join(work_dir, '02_scATAC/consensus_peak_calling/pseudobulk_bed_files/bed_paths_new.pkl'), 'wb'))
pickle.dump(bw_paths,
           open(os.path.join(work_dir, '02_scATAC/consensus_peak_calling/pseudobulk_bed_files/bw_paths_new.pkl'), 'wb'))

# Call peaks per pseudobulk profile
import pickle
bed_paths = pickle.load(open(os.path.join(work_dir, '02_scATAC/consensus_peak_calling/pseudobulk_bed_files/bed_paths_new.pkl'), 'rb'))
bw_paths =  pickle.load(open(os.path.join(work_dir, '02_scATAC/consensus_peak_calling/pseudobulk_bed_files/bw_paths_new.pkl'), 'rb'))
...
pickle.dump(narrow_peaks_dict,
            open(os.path.join(work_dir, '02_scATAC/consensus_peak_calling/MACS/narrow_peaks_dict_new.pkl'), 'wb'))
...
pickle.dump(cistopic_obj,
            open(os.path.join(work_dir, '02_scATAC/cistopic_obj_by_signac_peak.pkl'), 'wb'))
...
pickle.dump(models,
            open(os.path.join(work_dir, '02_scATAC/models/10x_ORN_models_500_iter_LDA_by_signac_peak.pkl'), 'wb'))

models = pickle.load(open(os.path.join(work_dir, '02_scATAC/models/10x_ORN_models_500_iter_LDA_by_signac_peak.pkl'), 'rb'))
cistopic_obj = pickle.load(open(os.path.join(work_dir, '02_scATAC/cistopic_obj_by_signac_peak.pkl'), 'rb'))
...
model = evaluate_models(models,
                       ...
                       plot_metrics=False)
plt.savefig(os.path.join(work_dir, '02_scATAC/models/evaluate_models_cistopic_obj_by_signac_peak.png'))
plt.show()
...
pickle.dump(cistopic_obj,
            open(os.path.join(work_dir, '02_scATAC/cistopic_obj_by_signac_pea.pkl'), 'wb'))
...
plt.savefig(os.path.join(work_dir, '02_scATAC/metadata_umap_by_signac_pea.pdf'))
plt.show()
plot_topic(cistopic_obj, reduction_name = 'UMAP', num_columns = 4)
plt.savefig(os.path.join(work_dir, '02_scATAC/topics_top_16_umap_by_signac_pea.pdf'))
plt.show()
...
# Step6: Downstream analysis for honeybee motif 
...
region_ranking = dill.load(open(os.path.join(work_dir, '04_scenicplus/region_ranking.pkl'), 'rb')) #load ranking calculated using the wrapper function
gene_ranking = dill.load(open(os.path.join(work_dir, '04_scenicplus/gene_ranking.pkl'), 'rb')) #load ranking calculated using the wrapper function
...
plt.savefig(os.path.join(work_dir, '04_scenicplus/metadata_umap_by_signac_pea_.pdf'))
plt.show()
...
plt.savefig(os.path.join(work_dir, '04_scenicplus/select_eRegulons_correlations.pdf'))
plt.show()
...
dill.dump(scplus_obj, open(os.path.join(work_dir, '04_scenicplus/scplus_obj_select.pkl'), 'wb'), protocol=-1)
...
plt.savefig(os.path.join(work_dir, '04_scenicplus/select_eRegulons_dotplot_heatmap.pdf'))
plt.show()
...
plot_rss(scplus_obj, 'GEX_subcluster_filtered', num_columns=2, top_n=10, figsize = (5, 10))
plt.savefig(os.path.join(work_dir, '04_scenicplus/select_TF_RSS.pdf'))
plt.show()

flat_list = lambda t: [item for sublist in t for item in sublist]
selected_markers = list(set(flat_list(
    [scplus_obj.uns['RSS']['GEX_subcluster_filtered'].loc[celltype].sort_values(ascending = False).head(10).index.to_list()
    for celltype in scplus_obj.uns['RSS']['GEX_subcluster_filtered'].index])))
...
plt.savefig(os.path.join(work_dir, '04_scenicplus/select_TF_RSS_heatmap.pdf'))
plt.show()

from pycisTopic.diff_features import find_highly_variable_features
hvr = find_highly_variable_features(scplus_obj.to_df('ACC').loc[list(set(scplus_obj.uns['eRegulon_metadata_filtered']['Region']))], n_top_features=100, plot = False)
hvg = find_highly_variable_features(scplus_obj.to_df('EXP')[list(set(scplus_obj.uns['eRegulon_metadata_filtered']['Gene']))].T, n_top_features=98, plot = False)

from scenicplus.networks import create_nx_tables, create_nx_graph, plot_networkx, export_to_cytoscape
nx_tables = create_nx_tables(
    scplus_obj = scplus_obj,
    eRegulon_metadata_key ='eRegulon_metadata_filtered',
    subset_eRegulons = ['LOC100578885', 'LOC725189', 'Dll','LOC551460','LOC725081'],
    subset_regions = hvr,
    subset_genes = hvg,
    add_differential_gene_expression = True,
    add_differential_region_accessibility = True,
    differential_variable = ['GEX_subcluster'])
...
plot_networkx(G, pos)
plt.savefig(os.path.join(work_dir, '04_scenicplus/show_select_TF_network.pdf'))
plt.show()
...
null = open(os.devnull,'wb')
import dill
scplus_obj = dill.load(open(os.path.join(work_dir, '05_scenicplus_flydb/scplus_obj_flybase_motif.pkl'), 'rb'))
...
sns.despine(ax = axs[0]) #remove top and right edge of axis border
sns.despine(ax = axs[1]) #remove top and right edge of axis border
plt.savefig(os.path.join(work_dir, '05_scenicplus_flydb/metadata_umap_by_signac_pea_.pdf'))
plt.show()
