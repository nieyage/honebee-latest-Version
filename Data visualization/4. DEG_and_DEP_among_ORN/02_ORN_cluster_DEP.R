...
write.csv(da_peaks,"./05_ORN_cluster/06_DEG_and_DEP/DEP_FindAllMarkers.csv")
#verification
...
pdf("./05_ORN_cluster/06_DEG_and_DEP/DEP_FindAllMarkers_ORN_heatmap_avg.pdf",width=15,height=15)
...
pdf("./05_ORN_cluster/06_DEG_and_DEP/DEP_tau_density.pdf",width=10,height=5)
...
tau1_peak<-data$peak
write.csv(data,"./05_ORN_cluster/06_DEG_and_DEP/DEP_tau-0.9_cluster_specfic_data_peak_ORN.csv")
# plot the tau cluster specfic genes heatmap 
# plot the avg heatmap 
peak_Avg <- AverageExpression(ORN,features=tau1_peak)
count=t(scale(t(peak_Avg$peaks),scale = T,center = F))
count<- count[tau1_peak,]
pdf("./05_ORN_cluster/06_DEG_and_DEP/DEP_tau_ORN_heatmap_avg.pdf",width=15,height=15)
pheatmap(count,cluster_cols = F,cluster_rows = F,color = colorRampPalette(c("white", "firebrick3"))(100),show_rownames=F,show_colnames=T)
pheatmap(count,cluster_cols = F,cluster_rows = F,color = colorRampPalette(c("white", "#E41A1C"))(100),show_rownames=F,show_colnames=T)
pheatmap(count,cluster_cols = F,cluster_rows = F,color = colorRampPalette(c("#377EB8", "white", "#E41A1C"))(100),show_rownames=F,show_colnames=T)
dev.off();
...
markers <-read.csv("./05_ORN_cluster/06_DEG_and_DEP/DEP_FindAllMarkers_gene_peak_ORN.csv",row.names=1)
tau_data<-read.csv("./05_ORN_cluster/06_DEG_and_DEP/DEP_tau-0.9_cluster_specfic_data_peak_ORN.csv",row.names=1)
...
pdf("./05_ORN_cluster/06_DEG_and_DEP/DEP_2methods_venn..pdf")
for (i in levels(ORN)){
    cluster<-i;
    # 1.FindAllMarkers:
...
pdf("./ORN/07_DEG_and_DEP/tau_DEP_motif_heatmap.pdf",width=20,height=20)
