...
write.csv(markers,"/md01/nieyg/project/honeybee/honebee-latest-Version/05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/FindAllMarkers_gene.csv")
...
pdf("/md01/nieyg/project/honeybee/honebee-latest-Version/05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/DEG_FindAllMarkers_heatmap_top5.pdf",width=15,height=15)
...
pdf("/md01/nieyg/project/honeybee/honebee-latest-Version/05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/DEG_FindAllMarkers_heatmap_avg_log2FC1.pdf",width=15,height=15)
pheatmap(count,cluster_cols = F,cluster_rows = F,color = colorRampPalette(c("white", "firebrick3"))(100),show_rownames=T,show_colnames=T)
pheatmap(count,cluster_cols = F,cluster_rows = F,color = colorRampPalette(c("white", "#E41A1C"))(100),show_rownames=T,show_colnames=T)
pheatmap(count,cluster_cols = F,cluster_rows = F,color = colorRampPalette(c("#377EB8", "white", "#E41A1C"))(100),show_rownames=T,show_colnames=T)
dev.off();
...
pdf("./05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/DEG_tau_density.pdf",width=10,height=5)
...
gene_specific<-names(which(gene_tau>0.92))
...
tau1_gene<-data$gene
write.csv(data,"./05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/DEG_tau-0.9_cluster_specfic_data.csv")
...
pdf("/md01/nieyg/project/honeybee/honebee-latest-Version/05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/DEG_tau_heatmap.pdf",width=15,height=15)
pheatmap(count,cluster_cols = F,cluster_rows = F,color = colorRampPalette(c("white", "firebrick3"))(100),show_rownames=T,show_colnames=T)
pheatmap(count,cluster_cols = F,cluster_rows = F,color = colorRampPalette(c("white", "#E41A1C"))(100),show_rownames=T,show_colnames=T)
pheatmap(count,cluster_cols = F,cluster_rows = F,color = colorRampPalette(c("#377EB8", "white", "#E41A1C"))(100),show_rownames=T,show_colnames=T)
dev.off();
...
library(ggplot2)
pdf("./05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/allgene_cv.pdf",width=10,height=4)
...
write.csv(Or_cor_data,"./05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/correlation_top10_data.csv")

cor_Avg <- AverageExpression(ORN,features=Or_cor_data$gene,assays = "raw_RNA")
library(pheatmap)
count=t(scale(t(cor_Avg$raw_RNA),scale = T,center = T))
pdf("/md01/nieyg/project/honeybee/honebee-latest-Version/05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/DEG_cor_heatmap.pdf",width=15,height=15)
pheatmap(count,cluster_cols = F,cluster_rows = F,color = colorRampPalette(c("white", "firebrick3"))(100),show_rownames=T,show_colnames=T)
pheatmap(count,cluster_cols = F,cluster_rows = F,color = colorRampPalette(c("white", "#E41A1C"))(100),show_rownames=T,show_colnames=T)
pheatmap(count,cluster_cols = F,cluster_rows = F,color = colorRampPalette(c("#377EB8", "white", "#E41A1C"))(100),show_rownames=T,show_colnames=T)
dev.off();
...
write.csv(Or_mi_data,"./05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/MI_top10_data.csv")

mi_Avg <- AverageExpression(ORN,features=Or_mi_data$gene,assays = "raw_RNA")
library(pheatmap)
count=t(scale(t(mi_Avg$raw_RNA),scale = T,center = T))
pdf("/md01/nieyg/project/honeybee/honebee-latest-Version/05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/DEG_MI_heatmap.pdf",width=15,height=15)
pheatmap(count,cluster_cols = F,cluster_rows = F,color = colorRampPalette(c("white", "firebrick3"))(100),show_rownames=T,show_colnames=T)
pheatmap(count,cluster_cols = F,cluster_rows = F,color = colorRampPalette(c("white", "#E41A1C"))(100),show_rownames=T,show_colnames=T)
pheatmap(count,cluster_cols = F,cluster_rows = F,color = colorRampPalette(c("#377EB8", "white", "#E41A1C"))(100),show_rownames=T,show_colnames=T)
dev.off();
...
markers <-read.csv("./05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/FindAllMarkers_gene.csv",row.names=1)
tau_data<-read.csv("./05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/DEG_tau-0.9_cluster_specfic_data.csv",row.names=1)
cor_data<-read.csv("./05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/correlation_top10_data.csv",row.names=1)
mi_data <-read.csv("./05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/MI_top10_data.csv",row.names=1)
...
pdf("./05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/DEG_4methods_upsetR.pdf")
...
gene.df <- bitr(rownames(ORN), fromType = "SYMBOL",toType =  c("ENTREZID","GO","EVIDENCEALL"),OrgDb = Apis_mellifera.OrgDb)
tau_data<-read.csv("./05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/DEG_tau-0.9_cluster_specfic_data.csv",row.names=1)
cor_data<-read.csv("./05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/correlation_top10_data.csv",row.names=1)
cor_data<- cor_data[,2:3]
all<- rbind(tau_data,cor_data)
all$cluster<- factor(all$cluster,levels=levels(ORN))
all<- all[order(all$cluster),]

gene_plot<- unique(all$gene)
ORN_avg<-AverageExpression(
       ORN,
       assays = "raw_RNA",
       features = gene_plot,
...
library(pheatmap)
count=t(scale(t(tau_Avg),scale = T,center = T))
which(gene_plot%in% all_receptor_gene)
which(gene_plot%in% TF_list)
type<- rep("other",length(gene_plot))
type[which(gene_plot%in% all_receptor_gene)]<- "OR"
type[which(gene_plot%in% TF_list)]<- "TF"

annotation_row<- data.frame(type)
rownames(annotation_row)<- gene_plot
ann_colors<-list(type = c("other"="white", "TF"="#5F3D69","OR"="#B781CC"))
labs.row <- rownames(annotation_row)
labs.row[-which(gene_plot%in% TF_list)] <- ""
bk <- c(seq(-7,-0.1,by=0.01),seq(0,7,by=0.01))

pdf("./05_ORN_cluster2/07_DEG_and_DEP/02_without_GRN/DEG_heatmap_label.pdf",width=12,height=15)
pheatmap(count,
  cluster_cols = F,
  cluster_rows = F,
  color = c(colorRampPalette(colors = c("#3D8CCC","white"))(length(bk)/2),colorRampPalette(colors = c("white","#CC1619"))(length(bk)/2)),
  legend_breaks=seq(-7,7,1),
  breaks=bk,
  show_rownames=T,
  labels_row = labs.row,
